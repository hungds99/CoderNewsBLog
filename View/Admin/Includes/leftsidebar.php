            <div class="left side-menu">
                <div class="sidebar-inner slimscrollleft">

                    <!--- Sidemenu -->
                    <div id="sidebar-menu">
                        <ul>
                            <li class="menu-title">Thanh Điều Hướng</li>

                            <li class="has_sub">
                                <a href="index.php?c=Dashboard&a=Home" class="waves-effect"><i class="mdi mdi-view-dashboard"></i> <span> Bảng điều khiển </span> </a>
                            </li>

                            <li class="has_sub">
                                <a href="javascript:void(0);" class="waves-effect"><i class="mdi mdi-format-line-style"></i> <span> Danh mục </span> <span class="menu-arrow"></span></a>
                                <ul class="list-unstyled">
                                    <li><a href="index.php?c=Category&a=Add">Thêm danh mục</a></li>
                                    <li><a href="index.php?c=Category&a=Manage">Quản lý danh mục</a></li>
                                </ul>
                            </li>

                            <li class="has_sub">
                                <a href="javascript:void(0);" class="waves-effect"><i class="mdi mdi-file"></i> <span> Bài viết </span> <span class="menu-arrow"></span></a>
                                <ul class="list-unstyled">
                                    <li><a href="index.php?c=Post&a=Add">Thêm bài viết</a></li>
                                    <li><a href="index.php?c=Post&a=Manage">Quản lý bài viết</a></li>
                                    <li><a href="index.php?c=Post&a=Trash">Bài viết bị xóa</a></li>
                                </ul>
                            </li>

                            <li class="has_sub">
                                <a href="javascript:void(0);" class="waves-effect"><i class="mdi mdi-comment"></i> <span> Bình luận </span> <span class="menu-arrow"></span></a>
                                <ul class="list-unstyled">
                                    <li><a href="index.php?c=Comment&a=Disapprove"> Bình luận đang chờ </a></li>
                                    <li><a href="index.php?c=Comment&a=Approve"> Bình luận được xác nhận</a></li>
                                </ul>
                            </li>

                        </ul>
                    </div>
                    <!-- Sidebar -->
                    <div class="clearfix"></div>

                </div>
                <!-- Sidebar -left -->

            </div>